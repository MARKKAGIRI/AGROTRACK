// screens/crops/AddCropScreen.js
...
import { addCrop, setAuthToken } from '../../services/cropApi';
import { useAuth } from '../../context/AuthContext';

const AddCropScreen = ({ navigation, route }) => {
  const { farmId } = route.params; // Pass farmId from previous screen
  const { token } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await addCrop(farmId, cropData);
      ...
      Alert.alert('Error', error.message || 'Failed to add crop cycle');
    } finally {
      setLoading(false);
    }
  };
