const { PrismaClient } = require("../generated/prisma");
const prisma = new PrismaClient();
const { validationResult } = require("express-validator");

const clearUserCache = async (userId) => {
  try {
    const key = `user:${userId}:farmContext`;
    await redisClient.del(key);
    console.log(`[Cache] Invalidated context for user ${userId}`);
  } catch (err) {
    console.error("[Cache] Error clearing cache:", err);
  }
};

// Get all farms for logged-in user
const getAllFarms = async (req, res) => {
  try {
    const userId = req.user.user_id;

    const farms = await prisma.farm.findMany({
      where: { ownerId: userId },
      include: {
        cropCycles: {
          include: {
            crop: true, // Include static crop info
          },
        },
        weatherLogs: true,
      },
    });

    const formattedFarms = farms.map((farm) => {
      const totalCrops = farm.cropCycles.length || 0;
      const totalHarvested =
        farm.cropCycles.filter((c) => c.harvestDate && c.status === "harvested").length || 0;
      const upcomingHarvest =
        farm.cropCycles.filter(
          (c) => c.harvestDate && new Date(c.harvestDate) > new Date() && c.status !== "harvested"
        ).length || 0;

      return {
        id: farm.id,
        name: farm.name,
        size: farm.size,
        unit: farm.unit,
        type: farm.type,
        location: farm.location,
        latitude: farm.latitude,
        longitude: farm.longitude,
        createdAt: farm.createdAt,
        updatedAt: farm.updatedAt,
        analytics: { totalCrops, totalHarvested, upcomingHarvest },
      };
    });

    return res.status(200).json({
      success: true,
      message: "Farms retrieved successfully",
      farms: formattedFarms,
    });
  } catch (error) {
    console.error("Get farms error:", error);
    return res.status(500).json({
      success: false,
      message: "Server error",
      error: error.message,
    });
  }
};

// Get a single farm by ID
const getSingleFarm = async (req, res) => {
  try {
    const { farmId } = req.params;
    const userId = req.user.user_id;

    const farm = await prisma.farm.findUnique({
      where: { id: farmId },
      include: {
        cropCycles: {
          include: {
            crop: true, // Include static crop info
          },
        },
        weatherLogs: true,
      },
    });

    if (!farm) {
      return res
        .status(404)
        .json({ success: false, message: "Farm not found" });
    }
    if (farm.ownerId !== userId) {
      return res.status(403).json({ success: false, message: "Unauthorized" });
    }

    return res
      .status(200)
      .json({ success: true, message: "Farm retrieved", farm });
  } catch (error) {
    console.error("Get farm error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Server error", error: error.message });
  }
};

// Add a new farm
const addFarm = async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty())
    return res.status(400).json({ errors: errors.array() });

  try {
    const userId = req.user.user_id;
    const {
      name,
      location,
      size,
      unit,
      type,
      latitude,
      longitude,
    } = req.body;

    if (!name || !location)
      return res
        .status(400)
        .json({ success: false, message: "Required fields missing" });

    const user = await prisma.user.findUnique({ where: { id: userId } });
    if (!user)
      return res
        .status(404)
        .json({ success: false, message: "User not found" });

    const farm = await prisma.farm.create({
      data: {
        name,
        location,
        size,
        unit,
        type,
        latitude,
        longitude,
        ownerId: userId,
      },
    });

    // invalidate user cache after adding a new farm
    await clearUserCache(userId);

    return res
      .status(201)
      .json({ success: true, message: "Farm created", farm });
  } catch (error) {
    console.error("Add farm error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Server error", error: error.message });
  }
};

// Update an existing farm
const updateFarm = async (req, res) => {
  try {
    const { farmId } = req.params;
    const {
      name,
      location,
      size,
      unit,
      type,
      latitude,
      longitude,
    } = req.body;

    if (
      !name &&
      !location &&
      !size &&
      !unit &&
      !type &&
      !latitude &&
      !longitude
    ) {
      return res
        .status(400)
        .json({ success: false, message: "No update fields provided" });
    }

    const errors = validationResult(req);
    if (!errors.isEmpty())
      return res.status(400).json({ errors: errors.array() });

    const userId = req.user.user_id;
    const farm = await prisma.farm.findUnique({ where: { id: farmId } });
    if (!farm)
      return res
        .status(404)
        .json({ success: false, message: "Farm not found" });
    if (farm.ownerId !== userId)
      return res.status(403).json({ success: false, message: "Unauthorized" });

    const updatedFarm = await prisma.farm.update({
      where: { id: farmId },
      data: {
        name: name ?? farm.name,
        location: location ?? farm.location,
        size: size ?? farm.size,
        unit: unit ?? farm.unit,
        type: type ?? farm.type,
        latitude: latitude ?? farm.latitude,
        longitude: longitude ?? farm.longitude,
      },
    });

    // invalidate user cache after updating a farm
    await clearUserCache(userId);

    return res
      .status(200)
      .json({ success: true, message: "Farm updated", farm: updatedFarm });
  } catch (error) {
    console.error("Update farm error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Server error", error: error.message });
  }
};

// Delete farm
const deleteFarm = async (req, res) => {
  try {
    const { farmId } = req.params;
    const userId = req.user.user_id;

    const farm = await prisma.farm.findUnique({ where: { id: farmId } });
    if (!farm)
      return res
        .status(404)
        .json({ success: false, message: "Farm not found" });
    if (farm.ownerId !== userId)
      return res.status(403).json({ success: false, message: "Unauthorized" });

    await prisma.farm.delete({ where: { id: farmId } });

    // invalidate user cache after deleting a farm
    await clearUserCache(userId);
    
    return res.status(200).json({ success: true, message: "Farm deleted" });
  } catch (error) {
    console.error("Delete farm error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Server error", error: error.message });
  }
};

module.exports = {
  getAllFarms,
  getSingleFarm,
  addFarm,
  updateFarm,
  deleteFarm,
};